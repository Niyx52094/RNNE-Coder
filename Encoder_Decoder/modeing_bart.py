...
class BaseModelOutputWithPast(ModelOutput):
    ...
    Args:
        ...
        attentions (:obj:`tuple(torch.FloatTensor)`, `optional`, returned when ``output_attentions=True`` is passed or when ``config.output_attentions=True``):
            ...
            heads.
        encoder_decoder_attentions：(:obj:`tuple(torch.FloatTensor)`, `optional`, returned when ``output_attentions=True`` is passed or when ``config.output_attentions=True``):
            Tuple of :obj:`torch.FloatTensor` (one for each layer) of shape
            :obj:`(batch_size, num_heads, sequence_length, sequence_length)`.

            Attentions weights after the attention softmax, used to compute the weighted average in the encoder_decoder
            heads.
    """
    ...
    encoder_decoder_attentions: Optional[Tuple[torch.FloatTensor]] = None
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        x, encoder_decoder_attn_weights = self.encoder_attn(
            query=x,
...
class BartDecoder(nn.Module):
    ...
    def forward(
        ...
        all_encoder_decoder_attns = () if output_attentions else None
        next_decoder_cache = []
        for idx, decoder_layer in enumerate(self.layers):
            ...
            x, layer_self_attn, layer_past, encoder_decoder_attn = decoder_layer(
                x,
                encoder_hidden_states,
            ...
            if output_attentions:
                ...
                all_encoder_decoder_attns +=(encoder_decoder_attn, )
        ...
        if not return_dict:
            return tuple(v for v in [x, next_cache, all_hidden_states, all_self_attns,all_encoder_decoder_attns] if v is not None)
        return BaseModelOutputWithPast(
            last_hidden_state=x, past_key_values=next_cache, hidden_states=all_hidden_states, attentions=all_self_attns,
            encoder_decoder_attentions=all_encoder_decoder_attns
        )
